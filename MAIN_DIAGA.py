								...
								try:
												...
												if gw.connect():
																...
																# если ара найдена на первом шлюзе, выводим результат
																				...
																				print(f"  ARP: {arp_mac if arp_mac else 'MAC не найден'} (на порту {len(mac_from_l2)} MAC-адресов, проверьте порт вручную)")

																elif arp_mac and arp_mac in mac_from_l2 and ip_found:
																				print("  ARP: OK (IP и MAC совпадают)")

																elif arp_mac:
																				print(f"  ARP: НЕ СООТВЕТСТВУЕТ (L2: {', '.join(mac_from_l2)} | L3: {arp_mac})")
