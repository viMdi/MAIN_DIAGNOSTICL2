...
class DLinkTelnetClient:
		...
		def check_utilization_cpu(self):
				...
				try:
						...
						# ищем загрузку цп
						res_util_cpu = re.search(r"Five Seconds.*?(\d+\s*%)", utilization_cpu)
						if res_util_cpu:
		...
		def check_cable_diagnostic(self, port):
				...
				try:
						...
						self.session.sendline("")
						self.session.expect(["5#", "admin#"], timeout=1)
						# отправляем команду cable diagnostic port
						self.session.sendline(f"cable diagnostic port {port}")

						"""добавить 2 промта на кабдиаг под свитчи"""

						time.sleep(0.6)
						self.session.expect(["5#", "admin#"], timeout=1)
						cab_diag = self.session.before.decode("utf-8", errors="ignore")
						"""здесь поправить регулярку + убрать лишние стрипы"""
						cab_diag_port = re.search(r"Link (?:Up|Down)\s+([^\n]+(?:\s+Pair\d.(?:OK|OPEN|SHORT|CROSSTALK)(?:\s+\s+\d+\s+M)?[^\n]*)*)", cab_diag, re.DOTALL | re.IGNORECASE)
						#
						if cab_diag_port:
